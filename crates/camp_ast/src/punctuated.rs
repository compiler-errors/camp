...
impl<T, S> Punctuated<T, S> {
    pub fn new() -> Punctuated<T, S> {
        Punctuated { items: vec![], last: None }
    }
    ...
    pub fn first(&self) -> Option<&T> {
        self.items.first().map(|(t, _)| t).or_else(|| self.last.as_deref())
    }

    pub fn last(&self) -> Option<&T> {
        self.last.as_deref().or_else(|| self.items.last().map(|(t, _)| t))
    }

    pub fn iter_items(&self) -> impl Iterator<Item = &'_ T> {
        self.items.iter().map(|(t, _)| t).chain(self.last.as_deref())
    }
